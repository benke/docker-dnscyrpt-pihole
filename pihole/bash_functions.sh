...
setup_dnsmasq_dns() {
    ...
    local DNS1=$(dig +short "${1:-8.8.8.8}")
    local DNS2=$(dig +short "${2:-8.8.4.4}")
...
wait_for_dns_server() {
    local DNS1=$(dig +short "${1:-8.8.8.8}")
    local DNS2=$(dig +short "${1:-8.8.4.4}")
    set -e
    echo -n '::: Waiting for DNS server: '
    until $(nc -uzv $DNS1 53) || $(nc -uzv $DNS2 53); do
        printf '.'
        sleep 5
    done
    set +e
    echo "::: DNS server available, starting..."
}
